...
const MyReports = () => {
  ...
  useEffect(() => {
    const fetchUserReports = async () => {
      if (user && user.primaryEmailAddress) {
        try {
          const response = await axios.get(`http://8okf4vjhsldv9fnacs4gf6iano.ingress.hurricane.akash.pub/api/user_reports/${user.primaryEmailAddress.emailAddress}`);
  ...
  const handleReRaise = async (reportId) => {
    try {
      await axios.put(`http://8okf4vjhsldv9fnacs4gf6iano.ingress.hurricane.akash.pub/api/reraise_case/${reportId}`);
          ...
          {reports.map((report) => (
            <div key={report.id} className="report-item">
              ...
              {report.status === 'solved' && (
                <button onClick={() => handleReRaise(report.id)} className='reraise'>Re-raise</button>
              )}
              {report.media_url && (

...
const ReportForm = ({ onReportSubmitted = () => {} }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await axios.post('http://8okf4vjhsldv9fnacs4gf6iano.ingress.hurricane.akash.pub/api/reports', formData, {
        headers: {
    ...
    <div className={`report-form-container ${isFormSubmitted ? 'submitted' : ''}`}>
      ...
      {isFormSubmitted ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="report-form">
          ...
          <div className={`camera-section ${isCameraActive ? 'active' : ''}`}>
            {!image ? (
              <>
